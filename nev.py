...
from pipeline import peval_pipeline, prefine_pipeline, MathAgentPipeline, latex_reformat_pipeline
...
def run(args):
    ...
    logging.info(f"Running prefine process pipeline with {len(problems)} problems")
    logs = []
...
def main():
    ...
    # for formatter utilities
    parser.add_argument('-ft', '--format_latex', type=str, default=None, help="Reformat the logs generated by mas to standard latex, main designed for o4-mini and o3. You need to specify the log directory with this argument.")
    ...
    if args.format_latex:
        latex_reformat_pipeline(
            log_dir=args.format_latex,
            reform_model=args.reform_model,
            workers=6 if args.workers < 6 else args.workers
        )
        return
